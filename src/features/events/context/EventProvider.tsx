...
import { Prompt } from '@/features/events/components/signup/PromptView';
import { fetchList, EventSignup } from '@/features/events/api/fetch-list';
import { subscribeToEventList } from '@/features/events/api/subscribe-to-list';
import {
  createContext,
  ReactNode,
  useCallback,
...
interface EventContextType {
   ...
   */
  readonly confirmedList: ReadonlyArray<EventSignup>;
   ...
   */
  readonly waitlist: ReadonlyArray<EventSignup>;
...
export const EventProvider = ({
  ...
  const [confirmedList, setConfirmedList] = useState<EventSignup[]>([]);
  const [waitlist, setWaitlist] = useState<EventSignup[]>([]);
  ...
  useEffect(() => {
    const unsub = subscribeToEventList(eventId, refreshList, setRealtimeConnected);
    return () => {
      unsub();
    };
  }, [eventId, refreshList]);
