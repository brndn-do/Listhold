...
import { z } from 'zod';
import { createOrg } from '@/features/orgs/api/create-org';
import { ServiceError, ServiceErrorMessage } from '@/features/_shared/types/serviceError';
    ...
    >
      <div>
        ...
        <div className='group relative z-0 mb-3 w-full'>
          ...
          {errors.name && (
            <div className='mt-1 w-full pl-2'>
              <InlineError size='xs' justify='start' content={errors.name.message} />
            </div>
          )}
        </div>
        ...
        <div className='group relative z-0 mb-3 w-full'>
          ...
          {errors.slug && (
            <div className='mt-1 w-full pl-2'>
              <InlineError size='xs' justify='start' content={errors.slug.message} />
            </div>
          )}
        </div>
        ...
        <div className='group relative z-0 mb-3 w-full'>
          ...
          {errors.description && (
            <div className='mt-1 w-full pl-2'>
              <InlineError size='xs' justify='start' content={errors.description.message} />
            </div>
          )}
        </div>
      </div>
      <div className='flex max-w-full flex-col gap-4'>
        {createError && <InlineError justify={'start'} content={createError} />}
        {!createError && (
          <div>
            <Button
              type='submit'
